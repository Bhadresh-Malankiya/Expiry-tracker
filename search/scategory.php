...
if(!$_SESSION["username"])
	...
	{	
		if(isset($_POST['btn']))
		   ...
		   $pro1 = mysqli_query($con,"SELECT `pname`, `pcategory`, `Quantity`, `price`, `pdate`, `pedate`, `prdate`,  `rdetails`, `pdesc` FROM `products` WHERE `username` =  '$username' AND `pcategory`= '$search' ");
		...
		elseif(isset($_POST['btn2']))
	       ...
	       $doc1 = mysqli_query($con,"SELECT `dname`, `dcategory`, `idate`, `dedate`, `drdate`, `ddesc` FROM `documents`  WHERE `username` =  '$username'  AND `dcategory` = '$search2' ");
		   }
	  else
		{
